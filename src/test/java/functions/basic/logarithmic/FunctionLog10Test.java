...
public class FunctionLog10Test {
    ...
    @ParameterizedTest
    @CsvFileSource(resources = "/mocks/functions/basic/logarithmic/log10.csv")
    public void log10TestWithMockLn(Double x, BigDecimal expected) {
        FunctionLog10 log10 = new FunctionLog10(lnMock);
        BigDecimal result = log10.calculate(x, accuracy);
        assertEquals(expected.floatValue(), result.floatValue(), accuracy);
    }

    @ParameterizedTest
    @CsvFileSource(resources = "/mocks/functions/basic/logarithmic/log10.csv")
    public void log10TestWithoutMocks(Double x, BigDecimal expected) {
        FunctionLog10 log10 = new FunctionLog10(new FunctionLn());
        BigDecimal result = log10.calculate(x, accuracy);
        assertEquals(expected.floatValue(), result.floatValue(), accuracy);
    }
}
